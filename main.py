...
def spider(max_pages):#paramter max page is passed so that we can specify the no. of pages of website we want to crawl
    ...
    while page <= max_pages:
        url = "https://buckysroom.org/trade/search.php?page=" + str(page) #the url we want to crawl and the incremented page no.
        ...
        #like if we want to grab only the titles of each item on the trade website page this object would make it easy as we can search according to the class of that specific item
        for link in soup.findAll('a' , { 'class' : 'item-name' }): #"a" is used for linking in html the ahref tag, find all the links with the class item-name - here item name is the class for the title of every trade item of the trade website url we have provided
            ...
            fullhref = "https://buckysroom.org" + href #the url obtained by href is just the item-name url only - contained within the 'trade' directory of the website we are using
            #for forming the complete url we would basically need the url till the trade directory forming a complete url
